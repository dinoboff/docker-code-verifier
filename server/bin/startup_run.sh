...
$DOCKER run -d --name "$SERVER_CONTAINER" -p 80:80 --restart="always" --link "$PYTHON_CONTAINER":python --link "$ANGULARJS_CONTAINER":angularjs "$SERVER_IMAGE":"$CLUSTER_VERSION"
